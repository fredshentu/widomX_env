...
/*Pseudo torque control for mx28 which does not support torque control
  mx28 has torque limited 0~1023. This function has been tested.*/
void mx28PseudoTorque(int id, int direction, int torque) {
    ax12SetRegister2(id, AX_TORQUE_LIMIT_L, torque);
    //Serial.println("setting the torquelimit and the torque direction");
    SetPosition(id, direction*4095);
}

/*12A has torque limit range 1-1023. This function has been tested*/
void ax12ApplyTorque(int id, int direction, int torque) {
    ax12SetRegister2(id, AX_TORQUE_LIMIT_L, torque);
    SetPosition(id, direction * 1023);
}

/*Mx 64 torque control, consist interface:0~1023 torque, int 0 or 1 indicate dir*/
void mx64Torque(int id, int direction, int torque) {
    mx_SetTorque(id, direction*1024+torque);
}

/*windowx torque control, provide a consistent interface*/
void widowxTorque(int id, int direction, int torque) {
    if (id == 1 or id == 4) {
        mx28PseudoTorque(id, direction, torque);
    }
    if (id == 2 or id == 3) {
        mx64Torque(id, direction, torque);
    }
}
